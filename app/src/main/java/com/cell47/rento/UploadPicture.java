...
public class UploadPicture extends AppCompatActivity {
    ...
    private boolean digits(String num)
    {
    ...
    private void postAd() {
        ...
        if ((et_desc.getText().equals(""))||(et_price.getText().length()==0)||et_title.getText().length()==0||imgUri==null||
                housenum.getText().length()==0||et_area.getText().length()==0 || phoneNumber.getText().length() == 0 || landmark.getText().length() == 0){
            Toast.makeText(this, "All fields are mandatory", Toast.LENGTH_SHORT).show();
        }
        else if(phoneNumber.getText().length() != 10 || !digits(phoneNumber.getText().toString()))
        {
            ...
            if (sharedPreferences.getString("postalcode", null)!=null){
                ...
                mDataBaseRefadmin.child("contact").setValue(phoneNumber.getText().toString());
                ...
                mDataBaseRefadmin.child("Booked").setValue("0");
                mDataBaseRefadmin.child("window").setValue(nw+"", "0");
                mDataBaseRefadmin.child("almirah").setValue(na+"", "0");
                mDataBaseRefadmin.child("pref").setValue(prefnum+"", "No Preference");
                FirebaseDatabase.getInstance().getReference().child("users").child(sharedPreferences.getString("phone", "1234567890"))
                        .child("Published").addValueEventListener(new ValueEventListener() {
                    @Override
                    public void onDataChange(@NonNull DataSnapshot dataSnapshot) {

                        Integer posted = Integer.valueOf(dataSnapshot.getValue(String.class));
                        posted += 1;
                        FirebaseDatabase.getInstance().getReference().child("users").child(sharedPreferences.getString("phone", "1234567890"))
                                .child("Published").setValue(String.valueOf(posted));
                    }
                ...
                //mDataBaseRefadmin.child("phoneNumber").setValue(phoneNumber);
